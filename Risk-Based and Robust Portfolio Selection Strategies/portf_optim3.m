...
clear all;
format long
global yearindex period
yearindex = 2008;

% Add paths
addpath('E:\Random Software\MATLAB\CPLEX Optimazation Studio\cplex\matlab\x64_win64');
addpath('E:\Random Software\MATLAB\IPOPT Optimization\Ipopt-3.11.8-linux64mac64win32win64-matlabmexfiles');
...
for (period = 1:N_periods)
   ...
   for(strategy = 1:N_strat)
      ...
      % Calculate the transaction cost, Calculate the amount of cash after reblancing in cash account
      rebalance_trans = curr_positions - x{strategy, period};
      transaction_cost = cur_prices * abs(rebalance_trans) * 0.005;
      ...
      if cash{strategy, period} < 0
          ...
          stock_to_change = I;
          stock_to_change_price = cur_prices(1, stock_to_change);
      ...
      if strategy == 6
        portf_value{strategy}(day_ind_start:day_ind_end) = data_prices(day_ind_start:day_ind_end,:) * x{strategy,period} + cash{strategy,period} - data_prices(1,:) * init_positions;
      else
...
plot(days, portf_value{5},'k')
plot(days, portf_value{6},'r')
plot(days, portf_value{7},'b')

legend('Buy and Hold','Equally Weighted','Min Variance','Max Sharpe Ratio','Equal Risk Contri','Leveraged Equal Risk','Robust Optimization')
...
% Find the weight of portfolio at each period for robust optimization 
weight_array =zeros(20,12);
j = 1; % First day
for i = 1:12
    shares_array = x{7,i};
    current_prices = data_prices(j,:);
    total_value = current_prices * shares_array + cash{7,i};
...
title('Portfolio Allocations of Robust Optim 2008-09')
xlabel('Period')
ylabel('Weight of Asset')

% Find the weight of portfolio at each period for strategy 3 minimum
% variance
weight_array =zeros(20,12);
j = 1; % First day
for i = 1:12
...
plot(0:12,weight_array_minvar)
legend(headers{1,1}(2:21,1))
title('Portfolio Allocations of Minimum Variance 2008-09')
xlabel('Period')
ylabel('Weight of Asset')

% Find the weight of portfolio at each period for strategy 4 maximum sharpe
% ratio
...
plot(0:12,weight_array_minvar)
legend(headers{1,1}(2:21,1))
title('Portfolio Allocations of Maximum Sharpe Ratio 2008-09')
xlabel('Period')
ylabel('Weight of Asset')
